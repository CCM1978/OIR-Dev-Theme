...
function supernova_meta_content(){
    ...
    $author .= '<span class="meta_author">';
    $author .= '<a title="'.__('View all posts by ', 'Supernova').get_the_author_meta( 'display_name' ).'" href="'.get_author_posts_url( get_the_author_meta( 'ID' ) ).'">'.get_the_author_meta( 'display_name' ).'</a>'; 
    $author .= '</span>';
    $author .= '</li>';
    
    $date  = '<li>';
    $date  .= '<span class="meta_date">'.get_the_date().'</span>';
    $date .= '</li>';
    
    $comment = '<li class="leave_comment"><span>';
